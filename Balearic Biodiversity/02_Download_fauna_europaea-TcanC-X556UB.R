...
# Check if the original list is complete, we search in Fauna Europaea all the species belonging to a specific genus
...
# Remove the species containing sp.
sp.gen <- sp.gen[sp.gen$nWords != 1, ]
...
faunaEuropaea <- merge(taxa.distribution, genus.distribution.p, by="taxa.fauna.eu", all = T)

faunaEuropaea <-distinct(faunaEuropaea)

# Save .csv
write.csv2(faunaEuropaea, paste0("./Lists/faunaEuropaea/Arachnida_faunaEuropaea_", Sys.Date(),".csv"), row.names = F, fileEncoding = "UTF-8")
write.csv2(genus.distribution.pa, paste0("./Lists/faunaEuropaea/Arachnida_faunaEuropaea_pa_", Sys.Date(),".csv"), row.names = F, fileEncoding = "UTF-8")
